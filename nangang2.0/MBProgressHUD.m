...
//    return [self initWithFrame:view.bounds];
    if (view.tag !=HDT_MBPRGRESSBAR_NOTCANCEL) {
        tapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hideHUDClick:)];
...
-(void)hideHUDClick:(id)s{
    NSLog(@"监听view点击事件，点击取消MBProgressHUD");
...
#endif
    if ([_delegate respondsToSelector:@selector(hudWasHidden:)]) {
        [_delegate performSelector:@selector(hudWasHidden:) withObject:self];
    }
